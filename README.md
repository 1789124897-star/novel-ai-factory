...
```
主题 → 叙事内核 → 四阶段小说生成 → TTS 语音合成 → 字级时间戳字幕
    → 背景视频拼接 → 字幕叠加 → 水印 → BGM 混音 → 成品视频
```

Web 界面一键操作,后台异步执行,进度实时刷新。
...
- **智能 TTS + 字幕** — edge-tts 流式合成,基于字级时间戳按标点断句生成 SRT
- **全自动视频** — 背景片段拼接 + 字幕叠加 + 半透明水印 + BGM 混音
- **轻量异步任务** — 自研任务管理器(threading + Lock),前端轮询任务状态
- **双入口** — Web 界面 + CLI 命令行
...
- **四层架构** — 路由 / 服务 / 任务 / 核心分层解耦,API 层保持薄路由,业务逻辑统一下沉服务层
